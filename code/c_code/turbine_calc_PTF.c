...
#define S_FUNCTION_NAME turbine_calc_PTF
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
...
/* Output Port  7 */
#define OUT_PORT_7_NAME      Td_calc
...
/* Parameter  2 */
#define PARAMETER_1_NAME      flow_coeff_turb
#define PARAMETER_1_DTYPE     real_T
#define PARAMETER_1_COMPLEX   COMPLEX_NO
/* Parameter  3 */
#define PARAMETER_2_NAME      eff_coeff
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    real_T        *Td_calc  = (real_T *)ssGetOutputPortRealSignal(S,7);
    real_T        *eta_t  = (real_T *)ssGetOutputPortRealSignal(S,8);
    ...
    const real_T  *flow_coeff_turb  = (const real_T *)mxGetData(PARAM_DEF1(S));
    const real_T  *eff_coeff  = (const real_T *)mxGetData(PARAM_DEF2(S));

    turbine_calc_PTF_Outputs_wrapper(pu, Tu, Fu, pd, Td, Fd, omegat, dmu, deu, dmbu, dmd, ded, dmbd, Tqt, Td_calc, eta_t, fs, p_width0, flow_coeff_turb, p_width1, eff_coeff, p_width2);
}
