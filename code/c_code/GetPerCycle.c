...
#define S_FUNCTION_NAME GetPerCycle
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
...
#define MDL_INITIALIZE_CONDITIONS
 ...
 {
   real_T *y0Cyc = ssGetDWork(S,0);
   real_T *Oldy0Cyc = ssGetDWork(S,1);
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    const real_T  *y0Cyc = (const real_T*) ssGetDWork(S,0);
...
static void mdlUpdate(SimStruct *S, int_T tid)
{
    ...
    real_T *y0Cyl = (real_T*)ssGetDWork(S,0);
    real_T *Oldy0Cyl = (real_T*)ssGetDWork(S,1);
    
	if (reset[0] == 1) {
        y0Cyl[0] = u0[0] - Oldy0Cyl[0];	
		Oldy0Cyl[0] = u0[0];
    }
}
