 ...
 *
 */
void compressor_calc_PTFV1_Outputs_wrapper(const real_T *pu,
                          const real_T *Tu,
                          const real_T *Fu,
                          const real_T *pd,
                          const real_T *Td,
...
dz1 = comp_flow_map[iend+(*nsp)*jstart] - comp_flow_map[istart+(*nsp)*jstart];
//dz11 = comp_eff_map[iend+(*nsp)*jstart] - comp_eff_map[istart+(*nsp)*jstart];
dz2 = comp_flow_map[iend+(*nsp)*jend] - comp_flow_map[istart+(*nsp)*jend];
//dz21 = comp_eff_map[iend+(*nsp)*jend] - comp_eff_map[istart+(*nsp)*jend];
z1 = comp_flow_map[istart+(*nsp)*jstart] + (dz1/dpr_intv)*(pic- pr_rep[istart]);
//z11 = comp_eff_map[istart+(*nsp)*jstart] + (dz11/dpr_intv)*(pic- pr_rep[istart]);
z2 = comp_flow_map[istart+(*nsp)*jend] + (dz2/dpr_intv)*(pic- pr_rep[istart]);
//z21 = comp_eff_map[istart+(*nsp)*jend] + (dz21/dpr_intv)*(pic- pr_rep[istart]);
dz_flow = z2 - z1;
//dz_eff = z21 - z11;
...
j = 1;
while ((mDot_rep[j] < dm_corr) && (j < *nflow))
{
    j++;
}        
jstart = j - 1;
jend = j;
dmDot_intv = mDot_rep[jend] - mDot_rep[jstart];
dz1 = comp_eff_map[iend+(*nflow)*jstart] - comp_eff_map[istart+(*nflow)*jstart];
dz2 = comp_eff_map[iend+(*nflow)*jend] - comp_eff_map[istart+(*nflow)*jend];
z1 = comp_eff_map[istart+(*nflow)*jstart] + (dz1/dpr_intv)*(pic- pr_rep[istart]);
z2 = comp_eff_map[istart+(*nflow)*jend] + (dz2/dpr_intv)*(pic- pr_rep[istart]);
dz_eff = z2 - z1;
*eta_ic = z1 + (dz_eff/dmDot_intv)*(dm_corr- mDot_rep[jstart]);
