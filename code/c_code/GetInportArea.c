...
#define S_FUNCTION_NAME GetInportArea
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
...
/* Parameter  2 */
#define PARAMETER_1_NAME      aCrank
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    real_T        *xP  = (real_T *)ssGetOutputPortRealSignal(S,3);
    ...
    const real_T  *aCrank  = (const real_T *)mxGetData(PARAM_DEF1(S));
    const real_T  *lambdaCrank  = (const real_T *)mxGetData(PARAM_DEF2(S));
    const real_T  *X  = (const real_T *)mxGetData(PARAM_DEF3(S));
    const real_T  *Y  = (const real_T *)mxGetData(PARAM_DEF4(S));
    const real_T  *r  = (const real_T *)mxGetData(PARAM_DEF5(S));
    const real_T  *nPort  = (const real_T *)mxGetData(PARAM_DEF6(S));
    const real_T  *stroke  = (const real_T *)mxGetData(PARAM_DEF7(S));

    GetInportArea_Outputs_wrapper(phi, aOpen, Cd, aEff, xP, nStroke, p_width0, aCrank, p_width1, lambdaCrank, p_width2, X, p_width3, Y, p_width4, r, p_width5, nPort, p_width6, stroke, p_width7);
}
